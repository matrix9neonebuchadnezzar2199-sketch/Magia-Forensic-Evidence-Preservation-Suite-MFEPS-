# 🔬 MFEPS — Magia Forensic Evidence Preservation Suite v2.4.0
...
[![CI coverage](https://img.shields.io/badge/CI%20coverage-backend%20%E2%89%A575%25-4CAF50)](./.github/workflows/ci.yml)
...
| 📊 ダッシュボード統計 | 日別ジョブ推移・スループット・出力形式分布（ECharts） |
| 🌓 ライト / ダークテーマ | 設定で切替可能（明環境・印刷向け） |
| 📁 ケース管理 | 案件・証拠品一覧、ジョブ履歴のドリルダウン |
| 📦 ZIP エクスポート | 完了ジョブのイメージ・ハッシュ・メタデータ等を1パッケージ化 |

### ✨ v2.4.0（Phase 10）ハイライト

- ダッシュボードに **統計グラフ**（成功率サマリ含む）を追加
- **ライトモード** CSS と設定 UI
- **ケース / 証拠品** 専用ページ（`/cases`、RBAC: viewer 以上）
- イメージング完了後 **ZIP エクスポート**（USB/HDD・光学ウィザード）
- バックエンド **テストカバレッジ CI ゲート 75%**（`pytest-cov`）
...
**MFEPS v2.4.0** — _Forensic Evidence Preservation, Reimagined._
...
| 📊 Dashboard analytics | Daily job trends, throughput, output format mix (Apache ECharts) |
| 🌓 Light / dark theme | User-selectable in Settings |
| 📁 Case management | Cases, evidence items, job history (`/cases`, RBAC) |
| 📦 ZIP export | Package image, hashes, metadata after job completion |

### Latest v2.4.0 (Phase 10)

Dashboard charts, light theme, cases page, post-job ZIP export, and **75%** minimum backend coverage in CI.
